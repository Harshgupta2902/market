...
router.get('/', async (req, res) => {
  try {
    // Retrieve link and source_table from query parameters
    const { link, source_table } = req.query;

    if (!link || !source_table) {
      return res.status(400).json({ error: 'Please provide both link and source_table parameters' });
    }
    ...
    console.error(`An error occurred while fetching data for IPO: ${error.message}`);
    return res.status(500).json({ error: `An error occurred while fetching data for IPO: ${error.message}`, link : link });
  }
});
...
//           INSERT INTO details (slug, tables, link, lists, source_table) 
//           VALUES (?, ?, ?, ?, ?)
...
//         await connection.execute(insertOrUpdateQuery, insertValues);
